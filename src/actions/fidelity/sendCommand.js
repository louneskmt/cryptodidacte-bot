...
async function sendCommand(params, args) {
  ...
  if (mentions.length > 1) return end(params, 'Please provide only one recipient.');
  if (mentions.length === 0) return end(params, 'Please provide one recipient.');
  ...
  if (!amount) {
    return end(params, insertVariablesInTemplate(messageTemplates.fidelity.error, { err: 'Please enter valid amount and retry.' }));
  }

  sendTokens(from, to, amount)
    .then((result) => end(params, insertVariablesInTemplate(messageTemplates.fidelity.sendOk, { to: to.screen_name, amount, balance: result.balance })))
